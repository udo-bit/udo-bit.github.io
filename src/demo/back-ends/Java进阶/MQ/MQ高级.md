...
![img](assets/1687334552247-cab38ab5-ae63-4f06-9ece-e9f244e3c170.jpeg)
...
![img](assets/1690366611659-d5c7f355-7ab1-4eb8-8488-13e1d98843ce.png)
...
![img](assets/1687341529298-150b401d-67f9-4958-acdb-0d3147b0532b.png)
...
![img](assets/1687348187394-21a3698a-277a-478b-8cb8-2ee5bc79207f.png)
...
![img](assets/1687348449866-dee08277-6bc9-4463-9cb8-95013e05a6a2.png)
...
![img](assets/1687351726363-c27337c1-cd6e-497e-96ad-ac55fe4cb9e4.png)
...
![img](assets/1687353601905-9b09c0df-1b03-49e1-95c8-437ef9f3cd81.png)
...
![img](assets/1687353771968-5c560b86-a1ae-4649-8597-c7ebfeffa9a5.png)
...
![img](assets/1687354083723-84971642-712d-42bc-ba65-6e3b3b33758c.png)

![image-20231127215550003](assets/image-20231127215550003.png)
...
注意:关闭 消息Confirm 和return 机制

```java
 /**
     * 发送消息测试 持久化性能
     * MessagePostProcessor 消息发送的后置处理器(消息发送前执行)
     * 1) 新建交换机 persistent.fanout
     * 2)) 新建交队列 persistent.queue  绑定
     * 3) 关闭 return 和confirm
     * 4) 发送 非持久化消息 测试 Pageout 现象
     * 5) 发送 持久化消息 测试 Pageout 现象
     * 6) 通过控制台发送 非持久化消息 ,重启mq 测试 消息丢失
     */
    @Test
    public void testPersistent(){
        for (int i = 0; i <1000000 ; i++) {
            rabbitTemplate.convertAndSend("persistent.fanout", "", "hello", new MessagePostProcessor() {
                @Override
...
`PageOut`. `PageOut`会耗费一段时间，并且会阻塞队列进程。因此在这个过程中RabbitMQ不会再处理新的消息，生产者的所有请求都会被阻塞。
...
![img](assets/1687421366634-1dfca4a6-2407-43c2-8e65-fd7ba9e660dc.png)
...
![img](assets/1687421880071-2a21369f-00b3-481d-a9bb-ce69a346ccc3.png)
...
![img](assets/1687422619364-b0e414b9-55fc-49f2-b7b5-de52b72a9f56.png)
...
- o.s.amqp…MessageConversionException: Can be thrown when converting the incoming message payload using a
  MessageConverter.
- o.s.messaging…MessageConversionException: Can be thrown by the conversion service if additional conversion is required
  when mapping to a @RabbitListener method.
...
![img](assets/1687489262801-36725872-cc98-470a-ab6b-85cfd9c1b0ce.png)

放行以后，由于抛出的是**消息转换异常**，因此Spring会自动返回`reject`，所以消息依然会被删除：

![img](assets/1687490335196-66d14c99-45c2-4113-8a36-94e33c3ce7d5.png)
...
![img](assets/1687489262801-36725872-cc98-470a-ab6b-85cfd9c1b0ce.png)放行以后，由于抛出的是业务异常，所以Spring返回`ack`
，最终消息恢复至`Ready`状态，并且没有被RabbitMQ删除：

![img](assets/1687490819965-f638194b-f956-4ad3-8cd4-2e03f5e43674.png)
...
![img](assets/1687490923673-6eca30c4-4cd0-4a92-b6d4-2766c0ad1746.png)
...
- 本地重试3次以后，抛出了`AmqpRejectAndDontRequeueException`异常。查看RabbitMQ控制台，发现消息被删除了，说明最后SpringAMQP返回的是
  `reject`
...
![image-20231128152044918](assets/image-20231128152044918.png)
我们需要修改`OrderServiceImpl`中的`markOrderPaySuccess`方法：
...
![img](assets/1687521150465-25b54b36-b64a-4b2d-90b7-8dff12fb075b.jpeg)
...
如图，有一组绑定的交换机（`ttl.fanout`）和队列（`ttl.queue`）。但是`ttl.queue`没有消费者监听，而是设定了死信交换机`dead.direct`
，而队列`dead.queue`则与死信交换机绑定，RoutingKey是ttl：

![image-20231127224645513](assets/image-20231127224645513.png)

publisher发送了一条消息，但最终consumer在30 秒后才收到消息。我们成功实现了**延迟消息**。
...
public class DeadLetterConfig {
    ...
    public Queue ttlQueue(){
        return  QueueBuilder
                .durable("ttl.queue") // 声明队列
                .deadLetterExchange("dead.direct") //// 声明队列 给队列绑定私信交换机
                .deadLetterRoutingKey("ttl") // 当队列中的消息 到期后变为死信时,修改消息的路由key 为ttl
                .build();
    }
     ...
     */
    @Bean
    public Binding bindingTTLExchangeAndQueue(Queue ttlQueue, FanoutExchange ttlFanoutExchange){
     ...
     */
    @Bean
    public Binding bindingDeadExchangeAndQueue(Queue deadQueue, DirectExchange deadDirectExchange){
        return BindingBuilder.bind(deadQueue).to(deadDirectExchange).with("ttl");
    }
...
/**
     ...
     */
    @Test
    public void testSentTTLMessage(){
        rabbitTemplate.convertAndSend("ttl.fanout", "", "我是超时订单O_1234567:+Time" + LocalTime.now(), new MessagePostProcessor() {
            @Override
            public Message postProcessMessage(Message message) throws AmqpException {
...
```java
   /**
    ...
    public void listenDealQueue4TTL(String message){
...
![img](assets/1687576610561-71355772-460c-4b7a-bf71-904b40bccdf9.png)

当然，也可以直接使用课前资料提供好的插件：

![img](assets/1687611117405-f30b7216-cbef-44fc-a8a9-b62c50ef2a06.png)
...
![img](assets/1687576988700-43b5d4ad-a77c-4463-bea4-4f3c0888ebe5.png)
...
![img](assets/1690343275577-b0f99b4a-40e2-40cf-8da2-11f0dfbd7d7c.jpeg)
...
![img](assets/1687593452790-58e296b7-0761-40f6-b4be-9c19bff9cd3e.jpeg)

由于我们要多次发送延迟消息，因此需要先定义一个记录消息延迟时间的消息体，处于通用性考虑，我们将其定义到`hm-common`模块下：

![img](assets/1687593306116-2a41b7c0-064c-463f-b109-fa05db8609e8.png)
...
![img](assets/1687593919687-52eb9aa6-6f80-4b49-ba32-bb608018e333.png)
...
![img](assets/1687595291593-378450c1-ef00-4cbf-9be8-235d1eea8e7c.png)
...
![img](assets/1687595921876-005c46d9-4278-411b-bfc1-c5e545949cd5.png)
...
![img](assets/1690352506454-23b445b7-3a34-458e-bba2-47528a06ea65.png)
...
![img](assets/1690343618777-60200e66-3734-439b-91fe-db8ea3eba148.png)
